...
class CompletionsProxy:
    ...
    def _apply_input_modifiers(
        self, messages: list[dict[str, str]], layer_states: dict[BaseScanner, str]
    ) -> None:
        """
        ...
        scanners (e.g. Canary), and updates the message list in-place.
        """
        ...
        for scanner in self._injectors:
            # The scanner does the logic; we just handle the plumbing.
            injector = cast(Injector, scanner)
            content_with_token, token = injector.inject(content)
            layer_states[scanner] = token

        # Update the message list in-place
        messages[idx]["content"] = content_with_token

    def _apply_output_validators(
        self, response: Any, layer_states: dict[BaseScanner, str]
    ) -> None:
        """
        ...
        for choice in response.choices:
            ...
            for scanner in self._scanners:
                token = layer_states.get(scanner)
                ...
                if hasattr(scanner, "clean") and token:
                    content = scanner.clean(content, token)
